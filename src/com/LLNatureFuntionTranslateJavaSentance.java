...
/**
 * @author Administrator  修改  LLNatureFuntionTranslateJava 没有 语句块的功能
 *    本项目研究  java 简单语法的解析
 *  stack 是词法分析栈
 *  strque 是输入串存贮的栈
 *  
 ...
 *   select(S->publicclasstype{Z})=first(publicclasstype{Z}) public
	...
	select (Z->PZ)=first(PZ)=public
	select (Z->ε)=follow(Z) }
 *  select(D->typeM)=first(typeM) type
 *   select(M->idN)=first(idN) id
 *   select(N->ε)= follow(N->ε) public,}
 ...
 *   
 */
public class LLNatureFuntionTranslateJavaSentance  {
